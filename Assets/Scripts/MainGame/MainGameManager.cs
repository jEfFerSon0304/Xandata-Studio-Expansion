...
public class MainGameManager : NetworkBehaviour
{
    ...
    [Header("Turn Order Popup")]
    public TurnOrderPopupUI turnOrderPopupUI;
    public Button turnOrderOverlayButton;
    ...
    private IEnumerator InitializeWhenReady()
    {
        ...
        turnOrderOverlayButton.onClick.RemoveAllListeners();
        turnOrderOverlayButton.onClick.AddListener(() => turnOrderPopupUI.TogglePanel());

        // 🟢 Request turn order if client
        // 🟢 Request turn order and character sync if client
        if (!IsServer && GameState.Instance != null)
        {
            Debug.Log("[MainGameManager] Requesting initial turn order from server...");
            GameState.Instance.RequestInitialTurnOrderServerRpc();

            // Also request character sync
